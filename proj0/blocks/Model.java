...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        for (int j = 0; j <= piece.height() - 1; j++) {
            for (int i = 0; i <= piece.width() - 1; i++) {
                if (piece.get(j,  i) && (get(j + row, i + col))) {
                    return false;
                }
            }
        }
    ...
    void place(Piece piece, int row, int col) {
        if (placeable(piece, row, col)) {
            for (int i = 0; i <= piece.height() - 1; i++) {
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        return (nrows * _width + ncols * _height) * _streakLength;
    }
