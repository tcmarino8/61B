...
class Board {
    ...
    /**total blocked spaces.*/
    private int _totalBlocked = 0;
    ...
    /** set the inner 7X7 board to empty.*/
    void setInner() {
        int level = 0;
        int midStart = 24;
        while (level < 7) {
            for (int j = 0; j < 7; j++) {
                set((j + midStart + 11*level), EMPTY);
            }
            for (int k = 7; k < 11; k++) {
                set((k + midStart + 11 * level), BLOCKED);
            }
            level++;
        }
    }
    /** set the 7X7 board to empty while setting the outside board to blocked.*/
    void setBoard() {
        int i = 0;
        int startMid = 24;
        int endMid = 101;
        while (i < _board.length - 1) {
            if (i < startMid || i >= endMid) {
                set(i, BLOCKED);
                i++;
            } else {
                int end = 77;
    ...
    boolean canMove(PieceColor who) {
        for (char k = 'a'; k <= 'g'; k += 1) {
            for (char z = '1'; z <= '7'; z++) {
                if (get(k, z) == who) {
                    char start = (char) (z - 2);
                    for (char i = (char) (k - 2); i <= (char) (k + 2); i++) {
                        for (char j = start; j <= (char) (z + 2); j++) {
                            if (get(i, j) == EMPTY) {
                                return true;
                            }
                        }
    ...
    /**check if no more moves can be made, hence there is a winner.
     * @param player - piececolor of current player
     * @return - will return winner*/
    boolean endGame(PieceColor player) {
        ...
        if (_numPieces[player.opposite().ordinal()] == 0) {
            _winner = player;
            return true;
        }
        return false;
    ...
    void makeMove(Move move) {
        ...
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                int ind = index(move.col1(), move.row1());
                if (get(neighbor(ind, i, j)) == _whoseMove.opposite()) {
                    set(neighbor(ind, i, j), _whoseMove);
                    _numPieces[_whoseMove.ordinal()]++;
    ...
    void undo() {
        ...
        while (_undoSquares.peek() != null) {
            ...
            if (_undoSquares.size() == 0) {
                unrecordedSet(index, EMPTY);
                _numPieces[color.ordinal()]--;
                break;
            }
            if (color == EMPTY) {
                PieceColor sneakPeek = _undoPieces.peek();
                unrecordedSet(index, sneakPeek);
                _numPieces[_whoseMove.opposite().ordinal()]++;
                _numJumps--;
            } else if (_undoSquares.peek() != null) {
                if (color == BLUE) {
                    unrecordedSet(index, RED);
                    _numPieces[RED.ordinal()]++;
                    _numPieces[BLUE.ordinal()]--;
                } else {
                    unrecordedSet(index, BLUE);
                    _numPieces[BLUE.ordinal()]++;
                    _numPieces[RED.ordinal()]--;
                }
            } else {
    ...
    void setBlock(char c, char r) {
        ...
        if (c != 'd' && r != '4') {
            _totalBlocked += 4;
            set(c, r, BLOCKED);
            set((char) ('d' + ('d' - c)), r, BLOCKED);
            set((char) ('d' + ('d' - c)), ((char) ('4' + ('4' - r))), BLOCKED);
            set(c, ((char) ('4' + ('4' - r))), BLOCKED);
        }
        if (c == 'd') {
            _totalBlocked += 2;
            set(c, r, BLOCKED);
     ...
     * use get to find the location of a 2D array and see if there is nothing
    part of it.* */
    int totalOpen() {
        int unblocked = 0;
