...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate, int targetYear) {
        if (numYears(targetYear) == 0){
            ...
            return presentValue + Math.abs(presentValue) * Math.pow((rate / 100), numYears(targetYear));
        }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        if (numYears(targetYear) == 0){
            ...
            return futureValue(perYear, rate, targetYear) + totalSavings(perYear, targetYear - 1, rate);
        }
    }
